...
def cmd_planlist_pon(who, msg):
	'"/planlist_pon" Nasz plan lekcji'
	
	sendpresence("Sprawdzam plan lekcji")
	systoone(who, _(pon))


pon = ["WF", "polski", "matematyka", "wos", "fizyka"]


def cmd_weather(who, msg):
	'"/weather" pogoda na dzis'
	yahoo_result = pywapi.get_weather_from_yahoo('PLXX0027')
	cos = str(yahoo_result)
	
	systoone(who, _("Temperatura wynosi " + cos[cos.find("temp")+9:cos.find("temp")+11] + "*C"))
